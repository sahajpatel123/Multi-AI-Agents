...
export function AgentSharePage() {
  ...
  const [harvardCopyStatus, setHarvardCopyStatus] = useState<'idle' | 'copied' | 'failed'>('idle');
  ...
  const [vancouverCopyStatus, setVancouverCopyStatus] = useState<'idle' | 'copied' | 'failed'>('idle');
  ...
  const [harvardDownloadStatus, setHarvardDownloadStatus] = useState<'idle' | 'done' | 'failed'>('idle');
  const [harvardDownloadFeedbackKey, setHarvardDownloadFeedbackKey] = useState(0);
  ...
  const [vancouverDownloadStatus, setVancouverDownloadStatus] = useState<'idle' | 'done' | 'failed'>('idle');
  const [vancouverDownloadFeedbackKey, setVancouverDownloadFeedbackKey] = useState(0);
  ...
  const [harvardCopyError, setHarvardCopyError] = useState<string | null>(null);
  ...
  const [vancouverCopyError, setVancouverCopyError] = useState<string | null>(null);
  ...
  const [vancouverDownloadError, setVancouverDownloadError] = useState<string | null>(null);
  const [bundleDownloadError, setBundleDownloadError] = useState<string | null>(null);
  const [harvardDownloadError, setHarvardDownloadError] = useState<string | null>(null);
  ...
  const [harvardCopyInFlight, setHarvardCopyInFlight] = useState(false);
  ...
  const [vancouverCopyInFlight, setVancouverCopyInFlight] = useState(false);
  ...
  const amaCopyBusyRef = useRef(false);
  const amaCopyRequestRef = useRef(0);
  const citationCopyRequestRef = useRef(0);
  const apaCopyBusyRef = useRef(false);
  const apaCopyRequestRef = useRef(0);
  const harvardCopyBusyRef = useRef(false);
  const harvardCopyRequestRef = useRef(0);
  ...
  const vancouverCopyBusyRef = useRef(false);
  const vancouverCopyRequestRef = useRef(0);
  ...
  useEffect(() => {
    ...
    amaCopyBusyRef.current = false;
    setAmaCopyInFlight(false);
    apaCopyRequestRef.current += 1;
    apaCopyBusyRef.current = false;
    setApaCopyInFlight(false);
    harvardCopyRequestRef.current += 1;
    harvardCopyBusyRef.current = false;
    setHarvardCopyInFlight(false);
    ...
    vancouverCopyRequestRef.current += 1;
    vancouverCopyBusyRef.current = false;
    setVancouverCopyInFlight(false);
    ...
    setHarvardDownloadStatus('idle');
    setHarvardDownloadFeedbackKey(0);
    ...
    setVancouverDownloadStatus('idle');
    setVancouverDownloadFeedbackKey(0);
    ...
    setVancouverDownloadError(null);
    setBundleDownloadError(null);
    setHarvardDownloadError(null);
  ...
  const amaText = useMemo(
    () =>
      report
        ? formatAgentReportAma({
            title: report.title,
            question: report.question,
            url: citationUrl,
            sharedAt: report.sharedAt,
  ...
  const harvardText = useMemo(
    () =>
      report
        ? formatAgentReportHarvard({
            title: report.title,
            question: report.question,
            url: citationUrl,
            sharedAt: report.sharedAt,
  ...
  useEffect(() => {
    if (amaCopyStatus === 'idle') return;
    const hold = amaCopyStatus === 'failed' ? 2800 : 1600;
    const t = window.setTimeout(() => {
      setAmaCopyStatus('idle');
      setAmaCopyError(null);
    }, hold);
  ...
  useEffect(() => {
    if (harvardCopyStatus === 'idle') return;
    const hold = harvardCopyStatus === 'failed' ? 2800 : 1600;
    const t = window.setTimeout(() => {
      setHarvardCopyStatus('idle');
      setHarvardCopyError(null);
    }, hold);
  ...
  useEffect(() => {
    if (vancouverCopyStatus === 'idle') return;
    const hold = vancouverCopyStatus === 'failed' ? 2800 : 1600;
    const t = window.setTimeout(() => {
      setVancouverCopyStatus('idle');
      setVancouverCopyError(null);
    }, hold);
  ...
  useEffect(() => {
    ...
    const hold = amaDownloadStatus === 'failed' ? 2800 : 2000;
    const t = window.setTimeout(() => {
      setAmaDownloadStatus('idle');
      setAmaDownloadError(null);
    }, hold);
    return () => window.clearTimeout(t);
  }, [amaDownloadFeedbackKey, amaDownloadStatus]);
  ...
  useEffect(() => {
    if (harvardDownloadStatus === 'idle') return;
    const hold = harvardDownloadStatus === 'failed' ? 2800 : 2000;
    const t = window.setTimeout(() => {
      setHarvardDownloadStatus('idle');
      setHarvardDownloadError(null);
    }, hold);
    return () => window.clearTimeout(t);
  }, [harvardDownloadFeedbackKey, harvardDownloadStatus]);
  ...
  useEffect(() => {
    if (vancouverDownloadStatus === 'idle') return;
    const hold = vancouverDownloadStatus === 'failed' ? 2800 : 2000;
    const t = window.setTimeout(() => {
      setVancouverDownloadStatus('idle');
      setVancouverDownloadError(null);
    }, hold);
    return () => window.clearTimeout(t);
  }, [vancouverDownloadFeedbackKey, vancouverDownloadStatus]);
  ...
  const handleCopyAma = async () => {
    if (amaCopyBusyRef.current || !amaText) return;
    amaCopyBusyRef.current = true;
    setAmaCopyInFlight(true);
    const requestId = ++amaCopyRequestRef.current;
    setAmaCopyStatus('idle');
    setAmaCopyError(null);
    try {
      const ok = await copyToClipboard(amaText);
      if (amaCopyRequestRef.current !== requestId) return;
      if (ok) {
        setAmaCopyStatus('copied');
      } else {
        setAmaCopyStatus('failed');
        setAmaCopyError('Could not copy the AMA citation — copy it manually instead.');
      }
    } catch {
      if (amaCopyRequestRef.current !== requestId) return;
      setAmaCopyStatus('failed');
      ...
      if (amaCopyRequestRef.current === requestId) {
        amaCopyBusyRef.current = false;
        setAmaCopyInFlight(false);
      }
    }
  ...
  const handleCopyHarvard = async () => {
    if (harvardCopyBusyRef.current || !harvardText) return;
    harvardCopyBusyRef.current = true;
    setHarvardCopyInFlight(true);
    const requestId = ++harvardCopyRequestRef.current;
    setHarvardCopyStatus('idle');
    setHarvardCopyError(null);
    try {
      const ok = await copyToClipboard(harvardText);
      if (harvardCopyRequestRef.current !== requestId) return;
      if (ok) {
        setHarvardCopyStatus('copied');
      } else {
        setHarvardCopyStatus('failed');
        setHarvardCopyError('Could not copy the Harvard citation — copy it manually instead.');
      }
    } catch {
      if (harvardCopyRequestRef.current !== requestId) return;
      setHarvardCopyStatus('failed');
      ...
      if (harvardCopyRequestRef.current === requestId) {
        harvardCopyBusyRef.current = false;
        setHarvardCopyInFlight(false);
      }
    }
  ...
  const handleCopyVancouver = async () => {
    if (vancouverCopyBusyRef.current || !vancouverText) return;
    vancouverCopyBusyRef.current = true;
    setVancouverCopyInFlight(true);
    const requestId = ++vancouverCopyRequestRef.current;
    setVancouverCopyStatus('idle');
    setVancouverCopyError(null);
    try {
      const ok = await copyToClipboard(vancouverText);
      if (vancouverCopyRequestRef.current !== requestId) return;
      if (ok) {
        setVancouverCopyStatus('copied');
      } else {
        setVancouverCopyStatus('failed');
        setVancouverCopyError('Could not copy the Vancouver citation — copy it manually instead.');
      }
    } catch {
      if (vancouverCopyRequestRef.current !== requestId) return;
      setVancouverCopyStatus('failed');
      ...
      if (vancouverCopyRequestRef.current === requestId) {
        vancouverCopyBusyRef.current = false;
        setVancouverCopyInFlight(false);
      }
    }
  ...
  const handleDownloadAma = () => {
    ...
    setAmaDownloadError(null);
    setAmaDownloadFeedbackKey((current) => current + 1);
    const stem = `agent-share-citation-${(report.title || report.question || 'report').slice(0, 40)}-ama`;
    const ok = downloadAmaFile(amaText, stem);
    if (ok) {
      setAmaDownloadStatus('done');
    } else {
      setAmaDownloadStatus('failed');
      setAmaDownloadError('Could not download the AMA citation — try Copy AMA instead.');
    }
  };
  ...
  const handleDownloadHarvard = () => {
    if (!report || !harvardText) return;
    setHarvardDownloadError(null);
    setHarvardDownloadFeedbackKey((current) => current + 1);
    const stem = `agent-share-citation-${(report.title || report.question || 'report').slice(0, 40)}-harvard`;
    const ok = downloadHarvardFile(harvardText, stem);
    if (ok) {
      setHarvardDownloadStatus('done');
    } else {
      setHarvardDownloadStatus('failed');
      setHarvardDownloadError('Could not download the Harvard citation — try Copy Harvard instead.');
    }
  };
  ...
  const handleDownloadVancouver = () => {
    if (!report || !vancouverText) return;
    setVancouverDownloadError(null);
    setVancouverDownloadFeedbackKey((current) => current + 1);
    const stem = `agent-share-citation-${(report.title || report.question || 'report').slice(0, 40)}-vancouver`;
    const ok = downloadVancouverFile(vancouverText, stem);
    if (ok) {
      setVancouverDownloadStatus('done');
    } else {
      setVancouverDownloadStatus('failed');
      setVancouverDownloadError('Could not download the Vancouver citation — try Copy Vancouver instead.');
    }
  };
              ...
              <div className="share-take__body">
                ...
                <div className="share-take__section">
                  ...
                  <div className="share-take__tools">
                    ...
                    <button
                      type="button"
                      className={`arena-btn arena-btn--secondary arena-btn--sm${amaCopyStatus === 'copied' ? ' is-success' : ''}${amaCopyStatus === 'failed' ? ' is-error' : ''}`}
                      onClick={() => void handleCopyAma()}
                      disabled={amaCopyInFlight}
                    >
                      {amaCopyInFlight
                        ? 'Copying…'
                        : amaCopyStatus === 'copied'
                          ? 'AMA copied'
                          : amaCopyStatus === 'failed'
                            ? 'Copy AMA failed'
                            : 'Copy AMA'}
                    </button>
                    <button
                      type="button"
                      className={`arena-btn arena-btn--secondary arena-btn--sm${amaDownloadStatus === 'done' ? ' is-success' : ''}${amaDownloadStatus === 'failed' ? ' is-error' : ''}`}
                      onClick={handleDownloadAma}
                    >
                      {amaDownloadStatus === 'done'
                        ? 'AMA downloaded'
                        : amaDownloadStatus === 'failed'
                          ? 'AMA download failed'
                          : 'Download AMA'}
                    </button>
                    <button
                    ...
                    <button
                      type="button"
                      className={`arena-btn arena-btn--secondary arena-btn--sm${harvardCopyStatus === 'copied' ? ' is-success' : ''}${harvardCopyStatus === 'failed' ? ' is-error' : ''}`}
                      onClick={() => void handleCopyHarvard()}
                      disabled={harvardCopyInFlight}
                    >
                      {harvardCopyInFlight
                        ? 'Copying…'
                        : harvardCopyStatus === 'copied'
                          ? 'Harvard copied'
                          : harvardCopyStatus === 'failed'
                            ? 'Copy Harvard failed'
                            : 'Copy Harvard'}
                    </button>
                    <button
                      type="button"
                      className={`arena-btn arena-btn--secondary arena-btn--sm${harvardDownloadStatus === 'done' ? ' is-success' : ''}${harvardDownloadStatus === 'failed' ? ' is-error' : ''}`}
                      onClick={handleDownloadHarvard}
                    >
                      {harvardDownloadStatus === 'done'
                        ? 'Harvard downloaded'
                        : harvardDownloadStatus === 'failed'
                          ? 'Harvard download failed'
                          : 'Download Harvard'}
                    </button>
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      className={`arena-btn arena-btn--secondary arena-btn--sm${vancouverCopyStatus === 'copied' ? ' is-success' : ''}${vancouverCopyStatus === 'failed' ? ' is-error' : ''}`}
                      onClick={() => void handleCopyVancouver()}
                      disabled={vancouverCopyInFlight}
                    >
                      {vancouverCopyInFlight
                        ? 'Copying…'
                        : vancouverCopyStatus === 'copied'
                          ? 'Vancouver copied'
                          : vancouverCopyStatus === 'failed'
                            ? 'Copy Vancouver failed'
                            : 'Copy Vancouver'}
                    </button>
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      className={`arena-btn arena-btn--secondary arena-btn--sm${vancouverDownloadStatus === 'done' ? ' is-success' : ''}${vancouverDownloadStatus === 'failed' ? ' is-error' : ''}`}
                      onClick={handleDownloadVancouver}
                    >
                      {vancouverDownloadStatus === 'done'
                        ? 'Vancouver downloaded'
                        : vancouverDownloadStatus === 'failed'
                          ? 'Vancouver download failed'
                          : 'Download Vancouver'}
                    </button>
                    <button
                      type="button"
                  ...
                  <span className="share-take__status" role="status" aria-live="polite">
                    ...
                    {harvardCopyStatus === 'copied' ? 'Harvard citation copied to clipboard. ' : ''}
                    ...
                    {vancouverDownloadStatus === 'done' ? 'Vancouver citation downloaded.' : ''}
