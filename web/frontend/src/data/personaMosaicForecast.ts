...
export interface MosaicForecastDecisionEntry {
  readonly id: string;
...
const COUNTER_KEY = 'arena:persona-mosaic-forecast:counter:v1';
const DECISIONS_KEY = 'arena:persona-mosaic-forecast:decisions:v1';
const DECISIONS_LIMIT = 50;

export function readMosaicForecastCounter(): number {
...
export function incrementMosaicForecastCounter(): number {
  const next = readMosaicForecastCounter() + 1;
...
export function clearMosaicForecastCounter() {
...
export function appendMosaicForecastDecision(entry: MosaicForecastDecisionEntry) {
  if (typeof window === 'undefined') return;
  try {
    const raw = window.localStorage.getItem(DECISIONS_KEY);
    const existing: MosaicForecastDecisionEntry[] = raw
      ? (JSON.parse(raw) as MosaicForecastDecisionEntry[])
      : [];
...
export function readMosaicForecastDecisions(): ReadonlyArray<MosaicForecastDecisionEntry> {
  if (typeof window === 'undefined') return [];
  try {
    const raw = window.localStorage.getItem(DECISIONS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as MosaicForecastDecisionEntry[];
...
export function clearMosaicForecastDecisions() {
  if (typeof window === 'undefined') return;
  try {
...
export interface MosaicForecastTally {
  readonly a: number;
  readonly b: number;
}

/** Pure — compute lifetime A vs B win tally from decision log. */
export function mosaicForecastWinTally(
  decisions: ReadonlyArray<MosaicForecastDecisionEntry>,
): MosaicForecastTally {
  let a = 0;
  let b = 0;
...
export type MosaicForecastMajorityLabel = 'unanimous' | 'strong' | 'split';

export interface MosaicForecastMajorityInfo {
  readonly label: MosaicForecastMajorityLabel;
...
export function mosaicForecastMajorityInfo(
  tally: MosaicForecastTally,
  winner: MosaicForecastPick,
