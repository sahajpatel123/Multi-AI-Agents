...
export interface MosaicDilemmaCouncilDecisionEntry {
  readonly id: string;
...
const COUNTER_KEY = 'arena:persona-mosaic-dilemma-council:counter:v1';
const DECISIONS_KEY = 'arena:persona-mosaic-dilemma-council:decisions:v1';
const DECISIONS_LIMIT = 50;

export function readMosaicDilemmaCouncilCounter(): number {
...
export function incrementMosaicDilemmaCouncilCounter(): number {
  const next = readMosaicDilemmaCouncilCounter() + 1;
...
export function clearMosaicDilemmaCouncilCounter() {
...
export function appendMosaicDilemmaCouncilDecision(
  entry: MosaicDilemmaCouncilDecisionEntry,
) {
  if (typeof window === 'undefined') return;
  try {
    const raw = window.localStorage.getItem(DECISIONS_KEY);
    const existing: MosaicDilemmaCouncilDecisionEntry[] = raw
      ? (JSON.parse(raw) as MosaicDilemmaCouncilDecisionEntry[])
      : [];
...
export function readMosaicDilemmaCouncilDecisions(): ReadonlyArray<MosaicDilemmaCouncilDecisionEntry> {
  if (typeof window === 'undefined') return [];
  try {
    const raw = window.localStorage.getItem(DECISIONS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as MosaicDilemmaCouncilDecisionEntry[];
...
export function clearMosaicDilemmaCouncilDecisions() {
  if (typeof window === 'undefined') return;
  try {
...
export interface MosaicDilemmaCouncilTally {
  readonly a: number;
  readonly b: number;
}

/** Pure — compute lifetime A vs B win tally from decision log. */
export function mosaicDilemmaCouncilWinTally(
  decisions: ReadonlyArray<MosaicDilemmaCouncilDecisionEntry>,
): MosaicDilemmaCouncilTally {
  let a = 0;
  let b = 0;
...
export type MosaicDilemmaCouncilMajorityLabel = 'decisive' | 'leaning' | 'split';

export interface MosaicDilemmaCouncilMajorityInfo {
  readonly label: MosaicDilemmaCouncilMajorityLabel;
...
export function mosaicDilemmaCouncilMajorityInfo(
  tally: MosaicDilemmaCouncilTally,
  winner: MosaicDilemmaCouncilPick,
