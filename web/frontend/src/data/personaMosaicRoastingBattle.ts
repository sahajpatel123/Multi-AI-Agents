...
export interface MosaicRoastingBattleDecisionEntry {
  readonly id: string;
...
const COUNTER_KEY = 'arena:persona-mosaic-roasting-battle:counter:v1';
const DECISIONS_KEY = 'arena:persona-mosaic-roasting-battle:decisions:v1';
const DECISIONS_LIMIT = 50;

export function readMosaicRoastingBattleCounter(): number {
...
export function incrementMosaicRoastingBattleCounter(): number {
  const next = readMosaicRoastingBattleCounter() + 1;
...
export function clearMosaicRoastingBattleCounter() {
...
export function appendMosaicRoastingBattleDecision(
  entry: MosaicRoastingBattleDecisionEntry,
) {
  if (typeof window === 'undefined') return;
  try {
    const raw = window.localStorage.getItem(DECISIONS_KEY);
    const existing: MosaicRoastingBattleDecisionEntry[] = raw
      ? (JSON.parse(raw) as MosaicRoastingBattleDecisionEntry[])
      : [];
...
export function readMosaicRoastingBattleDecisions(): ReadonlyArray<MosaicRoastingBattleDecisionEntry> {
  if (typeof window === 'undefined') return [];
  try {
    const raw = window.localStorage.getItem(DECISIONS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as MosaicRoastingBattleDecisionEntry[];
...
export function clearMosaicRoastingBattleDecisions() {
  if (typeof window === 'undefined') return;
  try {
...
export interface MosaicRoastingBattleTally {
  readonly a: number;
  readonly b: number;
}

/** Pure — compute lifetime A vs B win tally from decision log. */
export function mosaicRoastingBattleWinTally(
  decisions: ReadonlyArray<MosaicRoastingBattleDecisionEntry>,
): MosaicRoastingBattleTally {
  let a = 0;
  let b = 0;
...
export type MosaicRoastingBattleMajorityLabel = 'unanimous' | 'strong' | 'split';

export interface MosaicRoastingBattleMajorityInfo {
  readonly label: MosaicRoastingBattleMajorityLabel;
...
export function mosaicRoastingBattleMajorityInfo(
  tally: MosaicRoastingBattleTally,
  winner: MosaicRoastingBattlePick,
