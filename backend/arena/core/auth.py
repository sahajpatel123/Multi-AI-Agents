...
def orm_user_to_response(user: User, db: Optional[Session] = None) -> UserResponse:
    ...
    return UserResponse(
        ...
        loyalty_resume_attempts=int(getattr(user, "loyalty_resume_attempts", 0) or 0),
        loyalty_resume_next_attempt_at=getattr(user, "loyalty_resume_next_attempt_at", None),
