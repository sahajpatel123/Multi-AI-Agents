...
@pytest.mark.parametrize(
    "export_path",
    [
        "/api/agent/feedback/summary/export.csv",
        "/api/agent/feedback/summary/export.json",
        "/api/agent/feedback/summary/export.md",
    ],
)
async def test_feedback_summary_export_rate_limited(app_client, make_user, export_path):
    ...
    res = await app_client.get(
        export_path, headers=_headers(user)
    )
    assert res.status_code == 429, res.text[:300]
    assert res.json().get("detail", {}).get("error") == "rate_limit_exceeded"
    _clear()
